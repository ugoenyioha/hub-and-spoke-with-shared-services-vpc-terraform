...
module "shared_hosting_vpc" {

  for_each = {
    for k, v in var.vpcs : k => v
    if v.type == "shared-hosting-private"
  }
  ...
  subnets = {
    workload_vpcs = { cidrs = slice(each.value.workload_subnets, 0, each.value.number_azs) }
...
resource "aws_ram_resource_association" "shared-vpcs" {
  for_each = { for k, v in module.shared_hosting_vpc["shared-hosting-private-vpc"].private_subnet_attributes_by_az : k => v.arn }

  resource_arn       = each.value
  resource_share_arn = aws_ram_resource_share.shared-vpcs.id
}

resource "aws_ram_principal_association" "shared-vpcs" {
  principal          = "arn:aws:organizations::137414077032:ou/o-8tjp6asiwy/ou-unog-0asuqtbe" # <- hardcoding this for now
  resource_share_arn = aws_ram_resource_share.shared-vpcs.id
}

resource "aws_ram_resource_share" "shared-vpcs" {
  name                      = "shared-vpcs"
  allow_external_principals = false

  tags = {
    "shared-vpcs" = "development-environment"
  }
}
